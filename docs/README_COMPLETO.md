...
O chat usa duas chamadas principais de LLM:

```txt
Etapa 1: query_planner_agent
modelo atual: CHAT_PLANNER_MODEL=gpt-5.4-mini
funcao: entender a pergunta e gerar o plano JSON de ferramentas

Etapa 2: query_answer_agent
modelo atual: CHAT_MODEL=gpt-5.4
funcao: validar resultados, usar structured_query/file_search e redigir a resposta final
```

Há ainda uma chamada opcional para nomear conversas:

```txt
chat_title_agent
modelo atual: CHAT_TITLE_MODEL=gpt-5.4-nano
```

Fallbacks no código:

```txt
CHAT_PLANNER_MODEL -> CHAT_MODEL -> gpt-5.4-mini
