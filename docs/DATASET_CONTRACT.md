...
Comando manual, útil para reprocessar uma run antiga:

```bash
env UV_CACHE_DIR=/tmp/uv-cache uv run python app/scrapers/normalize_inferences.py scrape_results/inferences/<run>
```

Na pipeline completa atual, essa normalizacao ja roda automaticamente depois da etapa de inferencia. A execucao manual deve ser usada apenas quando quisermos corrigir uma run antiga ou testar novas regras sem rodar tudo de novo. A execucao atualiza os arquivos derivados da run informada e registra as mudancas em:
